...
def main(mode_sizes, num_initial, tolerance, num_proc, data_dir, config_dir):
    ...
    with open(config_dir + '/compress.txt', 'wt') as f:
        ...
        for s in mode_sizes[:-1]:
            f.write(' {:d}'.format(s))
        f.write(' {:d}\n'.format(num_initial))
        f.write('Grid dims                           = {:d} 1 1\n'.format(num_proc))
        f.write('Decompose mode order                = 0 1 2\n')
