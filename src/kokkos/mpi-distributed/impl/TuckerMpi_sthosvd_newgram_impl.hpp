...
auto newGram(Tensor<ScalarType, Properties...> & Y, int n)
{
  ...
  if(rank == 0) {
    std::cout << "____NEWGRAM____" << ": \n" ;
    Tucker::write_view_to_stream(std::cout, Y.getLocalTensor().data());
  }
  ...
  if(numProcs > 1)
  {
    ...
    if(myColEmpty) {
      ...
      if(rank == 0) {
	std::cout << "____NEWGRAM_2____" << ": \n" ;
	Tucker::write_view_to_stream(std::cout,
				     redistributedY.getLocalMatrix());
      }
				   ...
				   bool flipSign)
{
  ...
  using onnode_tensor_type = typename tensor_type::traits::onnode_tensor_type;
  using tucker_tensor_type = TuckerOnNode::TuckerTensor<onnode_tensor_type>;
  using slicing_info_view_t = Kokkos::View<TuckerOnNode::impl::PerModeSliceInfo*, Kokkos::HostSpace>;
  ...
  for (std::size_t n=0; n<X.rank(); n++)
  {
    ...
    TuckerOnNode::impl::appendEigenvaluesAndUpdateSliceInfo(mode, eigvals, currEigvals, perModeSlicingInfo(mode));
    if (mpiRank ==0){
    ...
    tensor_type temp = ttm(Y, mode, currEigVecs, true, max_lcl_nnz_x);
    Y = temp;
    MPI_Barrier(MPI_COMM_WORLD);
    if(mpiRank == 0) {
      ...
      Tucker::write_view_to_stream(std::cout, Y.getLocalSize());;
      Tucker::printBytes(local_nnz*sizeof(ScalarType));
      std::cout << "Global tensor size after STHOSVD iteration " << mode << ": \n";
      Tucker::write_view_to_stream(std::cout, Y.getGlobalSize());;
      Tucker::printBytes(global_nnz*sizeof(ScalarType));
    }

  }//end loop

  return tucker_tensor_type(Y.getLocalTensor(), eigvals, factors, perModeSlicingInfo);
}
