...
TEST(tuckerkokkos, compute_slice_metrics_2x2x2_mode2){
  // Prepare
...
TEST(tuckerkokkos, compute_slice_metrics_2x2x2_mode0){
  // Prepare
  typedef double scalar_t;
  using memory_space = Kokkos::DefaultExecutionSpace::memory_space;
...
TEST(tuckerkokkos, compute_slice_metrics_2x2x2_mode1){
  // Prepare
  typedef double scalar_t;
  using memory_space = Kokkos::DefaultExecutionSpace::memory_space;
...
TEST(tuckerkokkos, compute_slice_metrics_2x3x5x7_allmode){
  ...
  for(int i=0; i<nbr_dim; i++) {
    Tucker::MetricData<scalar_t> metric = Tucker::compute_slice_metrics(tensor, i, Tucker::MIN + Tucker::MAX + Tucker::SUM);
